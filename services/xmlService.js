...
<OcrConfiguration version="1.0" created="${now}">
    ...
    this.setElementText(doc, 'ModeOcr', config.modeOcr || 'auto'); 
    ...
    /**
     * Parse un document OCR XML et retourne les données structurées
     */
    ...
    try {
        ...
        const result = {
            ...
            modeOcr: this.getElementText(settings, 'ModeOcr') || 'auto',
    ...
    switch (settingName) {
        case 'PatchMode':
            ...
            break;
        case 'ModeOcr':
            const validOcrModes = ['auto', 'force', 'skip'];
            if (!validOcrModes.includes(value)) {
                errors.push(`ModeOcr "${value}" invalide. Modes supportés: ${validOcrModes.join(', ')}`);
            }
            break;
        case 'PatchNaming':
    ...
    /**
     * Synchronise le NamingPattern avec le FilePath d'un profil
     */
    async syncNamingPatternFromProfile(doc, profileDisplayName, profileService) {
        try {
            const filePath = await profileService.getFilePathFromProfile(profileDisplayName);
            this.setElementText(doc, 'NamingPattern', filePath);
            console.log(`NamingPattern synchronisé avec le profil "${profileDisplayName}": ${filePath}`);
            return filePath;
        } catch (error) {
            throw new Error(`Erreur synchronisation NamingPattern: ${error.message}`);
        }
    }
    ...
    try {
        ...
        if (settings && !settings.getElementsByTagName('ModeOcr')[0]) {
            const modeOcrElement = doc.createElement('ModeOcr');
